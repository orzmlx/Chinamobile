...
def parse_kpi(base_df, files, use_cols, on_index, date_index, date_offset, out_path):
    before_df_list = []
    after_df_list = []
    ...
    for i in tqdm(range(file_number)):
        ...
        before_df, after_df = select_data_by_date(base_df, file_df[use_cols + [on_index, date_index]], on_index,
                                                  date_offset, date_index)
...
if __name__ == "__main__":
    ...
    date_col = '数据开始时间'
    date_col_1 = '开始时间'
    # base_cols = ['数据开始时间', 'CGI']
    # base_cols_1 = ['开始时间', '小区CGI']
    ...
    g5_performance_cols = ['PDCP上行业务字节数(Mbyte)', 'PDCP下行业务字节数(Mbyte)', '小区UE上行速率小于1Mbps的样本数(个)',
                           '小区UE上行速率低于1Mbps样本比例(%)',
                           '小区UE下行速率小于5Mbps的样本数(个)', '小区UE下行速率低于5Mbps样本比例(%)']
    ...
    parse_kpi(g5_base_df, g5_performance_files, g5_performance_cols, on, date_col, 3, g5_performance_out_path)
    parse_kpi(g4_base_df, g4_performance_files, g4_performance_cols, on_1, date_col_1, 3, g4_performance_out_path)
