...
class HuaweiReporter:
    ...
    def __init__(self, data_path, to_be_checked_params, params_files_cols_dict, params_file_keys_dict, standard_path):
        """
            一个是文件名对应的列名字典
            一个是文件名中的key的字典
        """
    ...
    def read_standard(self):
        standard_alone_df = pd.read_excel(self.standard_path, sheet_name='4G自身推荐')
        # commands = standard_alone_df['主命令'].unique().tolist()
        command_grouped = standard_alone_df.groupby('主命令')
        for command, g in command_grouped:
            self.read_data_by_command(command, g)

    def read_data_by_command(self, command, g):
        ...
        for i in range(len(params) - 1, -1, -1):
            param = params[i]
            is_switch = g[g['原始参数名称'] == param]
            if is_switch:
                switch_params.append(param)
                params.remove(param)

        df = pd.read_csv(file_name, usecols=params)
    ...
    def generate_report(self):
        ...
        :return:
        """
        df = self.read_standard()
        for name, command, identity, recommended_value in zip(df['原始参数名称'], df['主命令'], df['标识'], df['推荐值']):
            result_df = self.search_param(name, command)

    def search_param(self, name, command):
        self.read_data_by_command(command)
...
if __name__ == "__main__":
    ...
    report = HuaweiReporter(filepaths, need_param, [], [], standard_path)
